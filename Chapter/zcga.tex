...
our specification and strong-type checkers already exists, and the
...
The \gls{zcga} can also be used on a semi-formal specification. The example we
use in this thesis describes an autopilot systems (appendix \ref{app:semiform}). This
...
figure \ref{fig:zcgaautopilotresult1}.
...
\begin{figure}[H]
    ...
    through the ZCGa checker. \label{fig:zcgaautopilotresult1}}
\end{figure}
...
formal, we are able to translate the annotated parts into the Isabelle
...
System Engineer A writes the following requirements:
 ...
 \noindent\fbox{%
     ...
     age\ of\ 18} \\
     \expression{sitting\ inside\ the\ vehicle}
     \item etc.
     \end{enumerate}
     }
 }
 ...
 \parbox{\textwidth}{%
 \begin{enumerate}
 \item The driver shall be sitting in a comfortable position in the vehicle.
 \item The \term{driver}: \expression{shall\ be\ a\ human\ being\ over\ the\ age\ of\ 17} \\
        \expression{sitting\ inside\ the\ vehicle}
