...
specification file, without any annoatation. Then we discuss the complexity of
...
We list information about how many different envronments and lines of \LaTeX{}
...
more lines of \LaTeX{} there is then the total amount of environments increse.
However, there are three exceptions to this trend. The `\emph{BirthdayBook}'
specification, `\emph{ClubState2}' specification and `\emph{Vendine Machine}'
...
\texttt{terms} used in the specification have to be intrduced by a \texttt{set}.
...
The table supports this informatio as there are more combined postconditions and
...
\cline{2-6}
 & \textbf{initiaOf} & \textbf{requires} & \textbf{allows} & \textbf{totalises}
 & \textbf{uses} \\
\hline
...
specifictions have an \emph{initialSchema} and therefore do not have an
...
It is made up of 507 lines of \LaTeX{} code, 10 zed environemnts, 34 schmes and
...
with four pumps and four pump controlers. There is a valve for emptying the
boiler.
...
The dependecy and goto graphs are shown in figures \ref{fig:steamdepgrap} and
\ref{fig:steamgotograph} respectively. Since there are a lot of \gls{zdra}
instances and therefore a lot of nodes, both the dependecy graph and goto graph
...
in the format decribed in chapter \ref{ch:skeletons} section \ref{subsec:pob1}.
...
first record. Middway through the theory file we see another record,
...
message telling the user that the specification is not correctlty totalised.
That is there is a precondition outstand with not postcondition counter part.
This does not matter for now as we can still carry on with the tranlation.

When checking the specification for \gls{zdra}, \gls{zmath} has also produced a
dependecy graph and goto graphs (shown in figures \ref{fig:autodepgraph} and
...
makes up the main tree dependencies. $OS1$ and $O1$ are seperate as they do not
have any relations which any parts of the main tree, the only dependency they
have is on eachother where $O1$ requires $OS1$.

We can say that the dependecy graph \emph{describes} the relation between
...
dependency on any part of the main tree they are seperate. However in the
...
woud need $OS1$ to appear before $O1$ since $O1$ requires $OS1$ to exist. We can
...
to exit. We can say that $PRE1$ requires $CS1$ and allows $PO1$. Thefore $PO1$
...
\gls{zmath} only checks for consistancy accross the specification and thus need
...
This can now give the user an idea of how to input their specifiation into
...
consistancy have been generated. The user can add the state Invariants in their
...
\Gls{zmath} is able assit the user with the translation of specification up to
...
so even thoguh we have chosen to prove our specification in certain ways other
...
stateinvariants and 2 changeSchemas, \gls{zmath} automatcally produces 2 lemma's
...
(\verb|w_max|). Therefore, the state invariants are writter after
`\texttt{assumes}' in the specification (\verb|w_min < w_max|). 

When \gls{zmath} produces the proof obigations to check the sanity of the
...
for the consistancy. To prove these lemmas we have in total used 18 tactics.

The lemmas to check that the changeSchema's did not conflict with
thestateInvariants with were as follows:
...
prove. The structure for the lemmas to check the consistancy of the telephone
...
check the stateinvairants are not conflicted when the state has been changed.
...
GenDB has 4 consistancy checks we must prove against the stateInvariants. The
...
Out of the 4 lemms we proved, 2 lemms can be proven by the tactic \verb|blast|.
The other 2 lemms we proved by using 5 other tactics. For example, we proved the
...
specification. The stateInvariants for the timtable specification are as
follows.
...
The stateInvairants taken from the ClubState2 state schema is
 ...
 schema uses the ClubState schema. Therefore the original stateInvairants
...
The ModuleReg specification has 2 changeSchemas and 1 set of stateInvairants.
...
To check the consistancy of all our specification we had to prove 56 lemmas with
...
Thefore a big chunk of work has been done. The advantage with this approach goes
...
system engineer may be able to prove the entire specification for consistancy
...
to prove the remainer of the proof. If the stakeholders require more properties
...
required if the stakeholders only want to check the system for consistancy and
the syntax of the specification is reletively easy where automated tools can be
...
the stakeholders require a more rigourous proof.

Automation on both sides are constantly being redeveloped and revisted. One can
...
mathematical notation is vast we can not incoporate the entire syntax of
...
\item Another assumtion the \gls{zmath} toolkit makes is that the document it is
...
for consistancy. As Stepney discussed in \cite{stepney1998tale} there are many
...
guidlines or codes of conduct it must obey. Proving properties to check for
consistancy is highly valuable, however other properties such as if
...
stage 6 and if the consistancy checks are not needed, they can be deleted from
...
\gls{zdra} checks if the specification is correctly totalised. Althought an
...
presented. If the specification is reletively small, with not a lot of instances
...
will determine how difficult the properites are to prove, whether they can be
...
toolkit and we disucss the achievments of the system and future work in the next
chapter.