...
\<or> (IsStudent projectalloc projectalloc' s outcome)
\<or> (IsLecturer projectalloc projectalloc' s outcome)"
apply (metis AddStudent_def TotalAddStudent insert_not_empty)
done
...
apply (smt IsLecturer_def IsStudent_def MESSAGE.distinct(23) MESSAGE.distinct(35) MESSAGE.distinct(9) SuccessMessage_def TotalAddLecturer)
done
...
apply (unfold NotAllocated_def SuccessMessage_def)
by (metis IsLecturer_def IsStudent_def MESSAGE.distinct(23) 
MESSAGE.distinct(35) MESSAGE.distinct(9) SuccessMessage_def TotalAddLecturer)
